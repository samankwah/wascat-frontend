...
function TeamPortrait({ member }: { member: TeamMember }) {
  return (
    <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden border-b border-[#d7e2e9] bg-[#dcecf3]">
      {member.image ? (
        <Image
        ...
        <span aria-hidden="true" className="display flex size-full items-center justify-center bg-[radial-gradient(circle_at_68%_24%,rgba(255,255,255,.7),transparent_28%),linear-gradient(145deg,#e7f3f8,#c6e0eb)] text-[clamp(3rem,5vw,4.5rem)] text-[#115273]">
          {member.initials}
        </span>
      )}
    </div>
...
export default function AboutPage() {
  return (
    <>
        ...
        <div className="container-shell">
          ...
          <ul className="mt-11 grid gap-5 md:grid-cols-2 lg:grid-cols-6" aria-label="WASCAT team members">
            {teamMembers.map((member, index) => (
              <li
                key={member.name}
                className={`group flex min-h-[390px] flex-col overflow-hidden border border-[#d7e2e9] bg-white shadow-[0_8px_24px_rgba(16,47,65,.05)] transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-1 hover:border-[#aac9d8] hover:shadow-[0_14px_34px_rgba(16,47,65,.11)] lg:col-span-2 ${index === 3 ? "lg:col-start-2" : ""} ${index === 4 ? "lg:col-start-4" : ""}`}
              >
                <TeamPortrait member={member} />
                <div className="flex flex-1 items-end justify-between gap-4 p-6">
                  <div>
                    <h3 className="display text-[1.55rem] leading-tight text-[#102433]">{member.name}</h3>
                  ...
                  {member.linkedin ? (
                    <a
                      ...
                      className="inline-flex size-10 shrink-0 items-center justify-center rounded-full border border-[#b8cbd5] text-[#1c6d99] transition-colors hover:border-[#0a66c2] hover:bg-[#0a66c2] hover:text-white focus-visible:border-[#0a66c2] focus-visible:bg-[#0a66c2] focus-visible:text-white"
                    >
                    ...
                    <span
                      ...
                      className="inline-flex size-10 shrink-0 cursor-not-allowed items-center justify-center rounded-full border border-[#cad7de] text-[#8497a3]"
                    >
                      <Linkedin size={18} strokeWidth={1.8} aria-hidden="true" />
                    </span>
                  )}
      ...
      <section className="bg-[#102f41] py-16 text-white"><div className="container-shell flex flex-col justify-between gap-7 md:flex-row md:items-center"><div><p className="eyebrow text-[#91c8e2]">QUESTIONS OR CORRECTIONS?</p><h2 className="display mt-3 text-4xl">Talk to the archive maintainers.</h2><p className="mt-3 text-sm text-[#bdd0da]">Include a stable image identifier or collection release in your message.</p></div><a href="mailto:data@wascat.org" className="button-light">data@wascat.org <ArrowRight size={16} /></a></div></section>
