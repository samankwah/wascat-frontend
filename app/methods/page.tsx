...
export default function MethodsPage() {
  return (
    <>
      <section className="bg-[#eaf4f8] py-16 md:py-24"><div className="container-shell grid gap-8 md:grid-cols-[.8fr_1.2fr]"><p className="eyebrow text-sky">FROM IMAGE TO COLLECTION</p><div><h1 className="display text-5xl leading-tight md:text-7xl">Measured, not labelled.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-muted">Every figure in the catalogue is derived from the delivered imagery: cloud cover is counted from the mask against the camera’s field of view, and each file carries its own checksum.</p><p className="mt-5 max-w-2xl text-sm leading-6 text-muted">Fields that depend on capture provenance — site, coordinates, timestamps, instrument — appear only where the capture team has supplied them, and are omitted otherwise rather than estimated.</p></div></div></section>
      ...
      <section className="bg-[#102f41] py-16 text-white md:py-24"><div className="container-shell grid gap-12 md:grid-cols-2"><div><p className="eyebrow text-[#91c8e2]">SEGMENTATION OUTPUT</p><h2 className="display mt-3 text-4xl">What a mask means</h2><p className="mt-5 leading-7 text-[#bdd0da]">Masks encode a binary cloud-versus-sky decision in the frame’s coordinate system. The overlay is a view composited in the browser; the mask is the analysis-ready artifact.</p></div><div className="grid grid-cols-2 gap-3 sm:gap-5">{[["white", "Cloud pixel"], ["black", "Sky pixel"], ["640×360", "Mask dimensions"], ["JPEG", "As delivered"]].map(([value, label]) => <div key={label} className="border border-white/15 p-4 sm:p-5"><strong className="display text-3xl text-[#d9ee9d]">{value}</strong><p className="mt-2 text-xs leading-5 text-[#bdd0da]">{label}</p></div>)}</div></div></section>
