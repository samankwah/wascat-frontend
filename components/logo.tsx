...
export function Logo({ inverse = false }: { inverse?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-3" aria-label="WASCAT home">
      <span aria-hidden="true" className={`relative block h-7 w-10 ${inverse ? "text-white" : "text-[#1c6d99]"}`}>
      ...
      <span className={`text-[1.04rem] font-bold tracking-[.17em] ${inverse ? "text-white" : "text-[#102433]"}`}>WASCAT <span className="text-[.62rem] tracking-[.08em] opacity-65">v1.0</span></span>
    </Link>
  );
}
